$(function () {
    ...
    function fInitialize() {
        var that = this;
        // 常用元素
        that.listEl = $('div.js-profile-list');
        // 初始化数据
        //that.uid = window.uid;
        that.page = 1;
        that.pageSize = 3;
                                ...
                                '<ul class="discuss-list">',
                                    '<li class="more-discuss">',
                                        '<a>',
                                            ...
                                            '<span> 条评论</span></a>',
                                    '</li>',
                                    '{% for content in image.comments.all() %}',
                                    '{% if loop.index >2 %}{% break %}{% endif %}',
                                    '<li>',
                                        '<a class=" icon-remove" title="删除评论"></a>',
                                        '<a class="_4zhc5 _iqaka" title="{{content.user.id}}" href="/profile/{{content.user.id}}" data-reactid=".0.1.0.0.0.2.1.2:$comment-17856951190001917.1">{{content.user.username}}</a>',
                                        '<span>',
